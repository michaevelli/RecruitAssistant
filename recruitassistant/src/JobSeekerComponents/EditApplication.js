...
export const retrieveApp="http://localhost:5000/retrieveapplication"
export const editApplicationUrl="http://localhost:5000/editapplication"

export default function EditApplication({match}) {
	...
	const getJob = async () => {
		...
		.then(res => {
			...
			const req_quals= res.data.jobinfo.req_qualifications.split(",")
			setQualificationList(req_quals)
			//initialise matching qualifications list to all false
			var initial_list=[]
			for (var i=0; i<req_quals.length; i++){
				if (res.data.applications.qualifications.indexOf(req_quals[i]) >= 0){
					initial_list.push(true)
				} else {
			...
			for (var i = 0; i < req_quals.length; i++){
				if (res.data.applications.qualifications.indexOf(req_quals[i]) >= 0){
					handleChangeQualification(i)
				}
			}*/
			
			setFirstName(res.data.applications.first_name)
			setLastName(res.data.applications.last_name)
			setPhoneNumber(res.data.applications.phone_number)
			setRights(res.data.applications.rights)
	...
	const applyJob = async () => {
		...
		await axios.post(url, data)
			.then(res => {
				...
				alert("Job application successfully edited")
			...
			<Row noGutters style={{height:'100%',paddingTop: 60}}>
				...
				<Col sm="10" >
					...
					<Form noValidate validated={validated} onSubmit={handleSubmit} style={{marginLeft:'15%'}}>          
						...
						<Form.Label column sm={10}>Do you have the rights to work in {detail.location}?</Form.Label>
							<Col sm={10}>
						...
						<Form.Group controlId="qualifications">
							...
							<Col sm={10}>
							{detail.req_qualifications.split(",").map((qualification, index) => (
								<Form.Check
									type = "checkbox"
									id = {qualification}
									label = {qualification}
						...
						<Form.Group controlId="required_docs">
							...
							<Col sm={10}>
								<ul>
									{detail.required_docs.split(",").map((document,index) => (
										<li>
											<Form.File
												required
												id = {index}
