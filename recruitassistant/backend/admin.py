...
def remove_list(data_list):
    dl = list(data_list.val().items())
    for i,j in dl:
        print(i)
        print(j)
...
def del_post():
    ...
    try:
        ...
        remove_list(db.child('offer').order_by_child('job_id').equal_to(post_id).get())
        
        # delete associated interviews
        remove_list(db.child('interviews').order_by_child('job_id').equal_to(post_id).get())
